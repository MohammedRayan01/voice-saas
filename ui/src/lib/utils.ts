...
export function setStackRefreshCookie(refreshToken: string) {
  ...
  const isLynqDomain = window.location.hostname.endsWith('.lynq.com');
  const cookieDomainPart = isLynqDomain ? '; domain=.lynq.com' : '';
...
export async function impersonateAsSuperadmin(params: {
  ...
  // (served from the target sub-domain, e.g. app.lynq.com). The route will set the
  ...
  // superadmin.lynq.com we want to switch to app.lynq.com. For any other domain
